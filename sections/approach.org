...
*** Task 1.2: Incorporate heterogeneous charger capabilities into the PAP to model slow and fast charging. Slow charging will be incentivized for battery health.
:PROPERTIES:
:CUSTOM_ID: sec:app-incorporate-pap
:END:

The kernel of this task was developed in [[#sec:queuing]] TODO: CONTINUE WRITING
...
a battery is shown in [[fig:soc-plot]] to demonstrate these components [cite:@zhang-2021-optim-elect].
...
*** Task 2.2: Model the PAP as an FFLP to allow for uncertain BEB arrival/charge/departure times. An optimal solution shall be produced, but with added flexibility to its application.
:PROPERTIES:
