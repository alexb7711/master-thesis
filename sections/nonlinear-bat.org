...
The following results are generated utilizing the same parameters and problem setup as [[#sec:sa-pap-example]]. The only
...
the non-linear battery dynamics as it performed better as shown in the results of [[#sec:sa-pap-example]].
...
80% (src_latex{\fpeval{388 * 0.8}} kWh), SOC increase per unit time begins to diminish whereas in the linear model it
