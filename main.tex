...
\appendixsection{State Of The Art of Battery Electric Bus Charging}
...
\appendixsection{Problem Description}
...
\appendixsection{Mixed Integer Linear Programming}
\label{sec:org87e56ec}
...
\appendixsection{Overview of the BAP}
...
\appendixsection{Overview of the PAP}
...
\appendixsection{Summary}
\label{sec:orgd97cc4e}
...
\appendixsection{Position Allocation Problem}
...
\appendixsubsection{Overview of BAP}
...
\appendixsubsection{The PAP Formulation}
\label{sec:the-pap-formulation}
The BAP forms the basis of the PAP; however, there are some differences in the way the variables are interpreted. The
starting service time, \(u_i\) seconds, is viewed as the initial charge time, and the service time, total elapsed time
spent on the charger. Similarly, for the spatial term, \(v_i \in [0,L]\), the berth location is instead interpreted as the
...
\appendixsection{A Rectangle Packing Formulation for BEB Charging}
...
\appendixsubsection{Queuing Constraints}
\label{sec:queuing}
...
\appendixsubsection{Battery Charge Dynamic Constraints}
...
\appendixsubsection{The BEB Charging Problem}
\label{sec:BEB_MILP}
...
\appendixsection{Optimization Problem}
...
\appendixsubsection{Variable Definitions}
...
\begin{enumerate}
\item Battery Dynamic Parameters
...
\item Packing and Discretization Parameters
...
\appendixsubsection{Constraints}
...
\appendixsection{Simulated Annealing}
...
\appendixsubsection{Cooling Equation}
...
\appendixsubsection{Acceptance Criteria}
...
\appendixsubsection{Neighbor Generators and Wrappers}
...
\appendixsubsection{Alternative Heuristic Implementation}
...
\appendixsection{Optimization Algorithm}
...
\appendixsubsection{BEB Scenario}
...
\appendixsection{Results}
\label{sec:org4611a61}
\appendix{Concluding Remarks}
\label{sec:conclusion}
\makeappendices
\appendix{The Fully Fuzzy Linear Program Model}
\label{sec:fuzzy-sa-pap}
The proposed work also intends to introduce a sense of robustness by allowing uncertainty in the arrival times of the
BEBs. By “fuzzifying” the model, the “fuzzy” variables are allowed to have upper and lower bounds for which the model
can provide solutions for. The work to be introduced promises much potential for further research and development in
regard to scheduling BEBs. What follows is a proposal for a Simulated Annealing (SA) implementation and extension of the
PAP utilizing Fully Fuzzy Mixed Integer Linear Programming (FFMILP) constraints with non-linear battery dynamics.
...
The appendix proceeds as follows. \ref{sec:fuzzy-preliminaries} introduces some of the basic concepts of Mixed Integer Linear
Programming (MILP), fuzzy set theory, and Fully Fuzzy Linear Programming (FFLP). \ref{sec:the-fuzzy-bap} introduces and
derives a Fuzzy BAP (FBAP) model. \ref{sec:the-fuzzy-pap} then adapts the PAP into the FPAP by utilizing the results from the
previous sections.
...
\appendixsubsection{Fuzzy Sets Theory}
...
\item Fuzzy Arithmetic
\label{sec:org502bb99}
...
\item Comparing Fuzzy Numbers
\label{sec:org5e3fde4}
...
\appendixsubsection{Fully Fuzzy Linear Programming}
